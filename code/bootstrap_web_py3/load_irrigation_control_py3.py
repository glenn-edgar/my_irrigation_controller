...
class Load_Irrigation_Pages(Base_Stream_Processing):
   ...
   def __init__( self, app, auth, request, app_files, sys_files,
       ...
       a1 = auth.login_required( self.irrigation_queue_front )
       app.add_url_rule('/ajax/irrigation_queue_front',"irrigation_queue_front",a1, methods=["POST"]) 
   ...
   def plc_stream(self):
       ...
       temp_data = self.handlers["PLC_MEASUREMENTS_STREAM"].revrange("+","-" , count=5000) # 1.5 days
       temp_data.reverse()
       '''
       filtered_data = []
       for i in temp_data:
   ...
   def irrigation_queue_front( self ):
       import msgpack
       h = self.handlers["IRRIGATION_PENDING"]
       h.redis_handle.rpush(h.key, msgpack.packb(self.request.json, use_bin_type=True))
