...
class Load_Irrigation_Statistics(Base_Stream_Processing):
   ...
   def __init__( self, app, auth, request, app_files, sys_files,
       ...
       a1= auth.login_required( self.irrigation_composite_statistics_table )
   ...
   def irrigation_composite_statistics(self):
       ...
       for i in valves:
            ...
            data_std.append(all_data[i][-1]["HUNTER_FLOW_METER"]["sd"])
            data_flow.append(all_data[i][-1]["HUNTER_FLOW_METER"]["mean"])
            ...
            ref_flow.append(mark_data["HUNTER_FLOW_METER"]["mean"])
            ref_std.append(mark_data["HUNTER_FLOW_METER"]["sd"])           
   ...
   def irrigation_composite_statistics_table(self):
       ...
       for i in range(0,len(valves)):
           valve_element = valves[i]
           if ref_flow[i]  == 0 :
                 continue
           ref_string = format_float = "{:.2f}".format(ref_flow[i])
           valve_data = all_data[valve_element][-1]["HUNTER_FLOW_METER"]["mean"]
           table_element =  table_element +  "<tr> <td>"+valve_element+ "</td><td>"+str(ref_string)
           table_element_a =  table_element_a +  "<tr> <td>"+valve_element+ "</td><td>"+str(ref_string)
           for j in range(0,8):
                 
                 if   j < len(all_data[valve_element]):
                     data_element = all_data[valve_element][-1-j]["HUNTER_FLOW_METER"]["mean"]
                     data_string = format_float = "{:.2f}".format(data_element)
                     table_element = table_element + "</td><td>"+data_string
                 else:
                     table_element = table_element + "</td><td> 0.0  "
                 if   j < len(all_data[valve_element]):
                     data_element_a = all_data[valve_element][-1-j]["HUNTER_FLOW_METER"]["sd"]
                     data_string = format_float = "{:.2f}".format(data_element_a)
                     table_element_a = table_element_a + "</td><td>"+data_string
                 else:
                     table_element_a = table_element_a + "</td><td> 0.0  "
           table_element = table_element +" </td> </tr>"
           table_element_a = table_element_a +" </td> </tr>"
    
    
       return self.render_template( "irrigation_statistics/irrigation_statistics_table",
                                   table_element = table_element,
                                   table_element_a = table_element_a,
                                   title = "Irrigation Composite History  Table",
                                    header = "Irrigation Composite History Table"
                                     )
