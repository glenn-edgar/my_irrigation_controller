...
class Station_Setup 
{
   ...
   populate_valves()
   {
        ...
        console.log("controller index  ",controller_index)
        var select_pins = this.controller_values[this.controller_list[controller_index]]
       console.log(select_pins)
        $("#station_valve").empty()
        for( let i = 0; i < select_pins.length; i++)
        {      
          var temp = select_pins[i]
          $("#station_valve").append($("<option></option>").val(temp).html("valve: "+temp))
